...
global mp Isat numutx numuty % Replace numut by 2 different mutation rates (Le Gland, 10/09/2019) 
...
global ntot0 
global temp0 sst temp % Use temperature at all depths (Le Gland, 15/10/2019) 
global jcounter 
global iTimeode 
...
% if sigmaxj > 0
    ...
    intPxyalfadxdy = (Ptot.^galfa ./ sqrt(galfa)) .* (sigmayj*sqrt(2*pi)).^(1-galfa);
...
dXYCOVdt = (xxvar.*xycov) .* (d2uxydxdx - d2gxydxdx - d2exydxdx) + (yyvar.*xycov) .* (d2uxydydy - d2gxydydy - d2exydydy) + ...
           (xxvar.*yyvar + xycov.^2) .* (d2uxydxdy - d2gxydxdy - d2exydxdy);
% dXYCOVdt = 0;
