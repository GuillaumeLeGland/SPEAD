function [varargout]=myregresslineplot(y,x,varargin) %si quiero "sombra" con std(y).
...
%TAKE OUT NaNs:
%%%%%%%%%%%%%%%
InoNaN=find(isnan(x)==0);
x=x(InoNaN);
y=y(InoNaN);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CHECK FOR EQUAL VALUES AND ADD A "MINI" DELTA INCREASE/DECREASE IF NECESSARY:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[xsort,xtag]=sort(x);
[ysort,ytag]=sort(y);

xdiff=abs(diff(xsort));
ydiff=abs(diff(ysort));

Isame=find(xdiff<=1d-6); %Positions for repeated values.
Jsame=find(ydiff<=1d-6);
...
xxsort=xsort;
if nsame>0
    xrand = rand(nsame,1)-1; %values between [0 - 1]
    dx = sqrt(eps)*xrand; %mini delta x fraction.
    xxsort(IIsame) = xsort(IIsame) + xsort(IIsame).*dx;
end

yysort=ysort;
if msame>0
    yrand = rand(msame,1)-1; %values between [0 - 1]
    dy = sqrt(eps)*yrand; %mini delta y fraction.
    yysort(JJsame) = ysort(JJsame) + ysort(JJsame).*dy;
end

xxsortrows=sortrows([xxsort,xtag],2);
yysortrows=sortrows([yysort,ytag],2);
xx=xxsortrows(:,1);
yy=yysortrows(:,1);
