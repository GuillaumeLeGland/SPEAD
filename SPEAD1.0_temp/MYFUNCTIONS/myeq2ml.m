function [eqml,param]=myeq2ml(eq,xvar)
...
%   'xdata'. The names of the parameters are returned in the cell array
...
%      returns 'm(1)+m(2)./xdata' and {'c','t_0'}
%
...
for i=1:maxm,
    for j=length(word):-1:1,
        ...
        elseif strcmp(word{j}, xvar),
            eqml = [eqml(1:(posword(j)-1)) 'xdata' eqml((posword(j)+length(xvar)):end)];
        end
    end
    [posword,word] = myfindword(eqml);
end
